...
Token Lexer::extract() {
    ...
    ignore(); //stream.unget() in previous versions
    ...
    throw std::runtime_error{"Unrecognized character detected."};
...
bool Lexer::peek(char& c) const {
    ...
    int x = input_stream.peek(); //std::istream doesn't allow us to peek(c) like it allowed get(c).
